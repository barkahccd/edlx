...
ACCOUNT_LOCK = threading.Lock()  # guards start/stop/restart per akun biar gak race
TG_OFFSET = 0
...
def parse_accounts_line_mode(text: str) -> dict[str, AccountConfig]:
    """
    Format simpel accounts.json, per akun dipisah baris kosong, isi 2 ATAU 3 baris:

    acc1
    akun1@gmail.com
    eyJ...              <- baris cookie OPSIONAL, boleh dilewatin kalau belum punya
    ...
    Baris kosong = pemisah antar akun (WAJIB ada di antara akun biar blok-nya kebaca benar).
    Baris komentar diawali # akan di-skip.
    Cookie boleh plain eyJ..., edel_session=eyJ..., full cookie header, atau dikosongkan sama
    sekali — nanti cookie asli tinggal dikirim belakangan lewat Telegram (@acc1 edel_session=...)
    atau diisi manual ke sessions/acc1.json.
    """
    ...
    # Kompatibilitas mundur: kalau seluruh file 1 blok gede tanpa baris kosong sama sekali
    # (format lama sebelum ada baris kosong pemisah) dan jumlah barisnya habis dibagi 3,
    # anggap format lama flat 3-baris/akun.
    if len(blocks) == 1 and len(blocks[0]) > 3 and len(blocks[0]) % 3 == 0:
        flat = blocks[0]
        blocks = [flat[i:i + 3] for i in range(0, len(flat), 3)]

    accounts: dict[str, AccountConfig] = {}
    for idx, block in enumerate(blocks, 1):
        if len(block) < 2:
            raise SystemExit(
                f"accounts.json blok akun ke-{idx}: minimal harus ada 2 baris (id, email), "
                f"cuma dapat {len(block)} baris: {block}. "
                "Cek baris kosong pemisah antar akun sudah benar belum."
            )
        if len(block) > 3:
            raise SystemExit(
                f"accounts.json blok akun ke-{idx}: kebanyakan baris ({len(block)}), "
                "maksimal 3 (id, email, cookie). Kemungkinan ada baris kosong pemisah "
                "yang kelewatan/salah taruh antar akun."
            )
        acc_id = normalize_account_id(block[0])
        email = block[1].strip()
        cookie = block[2].strip() if len(block) == 3 else ""
        if not acc_id:
            raise SystemExit(f"accounts.json blok akun ke-{idx}: id akun kosong")
...
def worker_thread(acc: AccountConfig, stop_event: threading.Event):
    try:
        with IMPORT_LOCK:
...
def start_workers():
    for acc in ACCOUNTS:
        ev = threading.Event()
        ACCOUNT_STOP_EVENTS[acc.id] = ev
        t = threading.Thread(target=worker_thread, args=(acc, ev), name=f"edel-{acc.id}", daemon=True)
        WORKER_THREADS[acc.id] = t
        t.start()
        log(f"thread @{acc.id} started")


def restart_single_account(acc_id: str):
    """Restart HANYA thread akun ini, thread akun lain tetap jalan seperti biasa."""
    acc_id = normalize_account_id(acc_id)
    acc = ACCOUNTS_BY_ID.get(acc_id)
    if not acc:
        log(f"restart @{acc_id} gagal: akun tidak ditemukan di accounts.json")
        return
    with ACCOUNT_LOCK:
        old_thread = WORKER_THREADS.get(acc_id)
        old_event = ACCOUNT_STOP_EVENTS.get(acc_id)
        if old_thread and old_thread.is_alive() and old_event:
            log(f"🔄 restart @{acc_id} diminta — menghentikan worker lama")
            old_event.set()
            old_thread.join(timeout=10)
            if old_thread.is_alive():
                log(f"⚠️ worker @{acc_id} lama belum berhenti dalam 10s, lanjut restart (worker lama akan exit sendiri belakangan)")

        new_event = threading.Event()
        ACCOUNT_STOP_EVENTS[acc_id] = new_event
        new_thread = threading.Thread(target=worker_thread, args=(acc, new_event), name=f"edel-{acc_id}", daemon=True)
        WORKER_THREADS[acc_id] = new_thread
        new_thread.start()
        log(f"✅ @{acc_id} restarted (akun lain tidak terganggu)")


def stop_single_account(acc_id: str):
    """Stop HANYA thread akun ini."""
    acc_id = normalize_account_id(acc_id)
    with ACCOUNT_LOCK:
        thread = WORKER_THREADS.get(acc_id)
        event = ACCOUNT_STOP_EVENTS.get(acc_id)
        if not thread or not thread.is_alive() or not event:
            log(f"stop @{acc_id}: worker tidak sedang jalan")
            return
        log(f"⏹ stop @{acc_id} diminta")
        event.set()
        thread.join(timeout=10)
        log(f"✅ @{acc_id} stopped")


def start_single_account(acc_id: str):
    """Start HANYA thread akun ini (kalau sebelumnya di-stop sendiri)."""
    acc_id = normalize_account_id(acc_id)
    acc = ACCOUNTS_BY_ID.get(acc_id)
    if not acc:
        log(f"start @{acc_id} gagal: akun tidak ditemukan di accounts.json")
        return
    with ACCOUNT_LOCK:
        thread = WORKER_THREADS.get(acc_id)
        if thread and thread.is_alive():
            log(f"start @{acc_id}: worker sudah jalan")
            return
        log(f"▶️ start @{acc_id} diminta")
        new_event = threading.Event()
        ACCOUNT_STOP_EVENTS[acc_id] = new_event
        new_thread = threading.Thread(target=worker_thread, args=(acc, new_event), name=f"edel-{acc_id}", daemon=True)
        WORKER_THREADS[acc_id] = new_thread
        new_thread.start()
        log(f"✅ @{acc_id} started")


def control_loop():
    """
    Baca perintah per-akun dari stdin (dikirim UI lewat process.stdin), format 1 JSON per baris:
      {"action": "restart_account", "account": "acc1"}
      {"action": "stop_account", "account": "acc1"}
      {"action": "start_account", "account": "acc1"}
    Ini yang bikin R/S/X di UI bisa target 1 akun aja, bukan seluruh proses manager.
    """
    try:
        for line in sys.stdin:
            ...
            if not acc_id:
                continue
            if action == "restart_account":
                restart_single_account(acc_id)
            elif action == "stop_account":
                stop_single_account(acc_id)
            elif action == "start_account":
                start_single_account(acc_id)
    except Exception:
        pass
...
def manager_main():
    ...
    tg.start()
    ctrl = threading.Thread(target=control_loop, name="stdin-control", daemon=True)
    ctrl.start()
    start_workers()
    try:
...
def run_integrated_ui():
    ...
    def _shorten_round(round_id):
        ...
        if len(round_id) > 30:
            return round_id[:14] + "…" + round_id[-10:]
        return round_id

    # Sama persis kayak tahapan di web asli (Calls Created → Allocation Pending → Calls Open →
    # Selections Submitted → Demand Index Pending → Demand Index Final), biar kolom Status di
    # dashboard gak nampilin raw enum API (SUBMITTED, SETTLED, dst) yang gampang kepotong.
    STATUS_LABELS_UI = {
        "CREATED": "📋 Calls Created",
        "NO_ROUND": "⚪ No Round",
        "ALLOCATION_PENDING": "⏳ Allocation Pending",
        "LOCK_PENDING": "⏳ Allocation Pending",
        "LOCKED": "⏳ Allocation Pending",
        "OPEN": "🟢 Calls Open",
        "SUBMITTED": "🔒 Selections Submitted",
        "DEMAND_INDEX_PENDING": "🔄 Demand Index Pending",
        "SETTLEMENT_PENDING": "🔄 Demand Index Pending",
        "SETTLEMENT_BUG": "🔄 Demand Index Pending",
        "SETTLED": "🏁 Demand Index Final",
    }

    def _status_label(raw_status):
        raw_status = str(raw_status or "-")
        return STATUS_LABELS_UI.get(raw_status, raw_status)

    def _votes_label(state):
        total = state.get("round_votes_total")
        picked = state.get("round_votes_picked")
        if not total:
            return "-"
        return f"{picked or 0}/{total}"
    ...
    def _session_status(acc_id):
        ...
        try:
            ...
            return "session ?"
        remaining = exp_ts - datetime.now(timezone.utc).timestamp()
        exp_wib = (datetime.fromtimestamp(exp_ts, tz=timezone.utc) + timedelta(hours=7)).strftime("%H:%M")
        if remaining <= 0:
            return f"expired ({exp_wib} WIB)"
        h = int(remaining // 3600)
        m = int((remaining % 3600) // 60)
        return f"ok {h}h{m}m (exp {exp_wib})"
    ...
    class EdelRunwayUI(App):
        ...
        BINDINGS = [
            ...
            ("shift+r", "restart_all_bot", "Restart ALL"),
        ...
        def compose(self) -> ComposeResult:
            ...
            with Vertical(id="bottom_panel"):
                yield Label("S=start akun | X=stop akun | R=restart akun terpilih | Shift+R=restart ALL | A=all logs | C=clear | Q=quit", id="log_title")
        ...
        def on_mount(self):
            ...
            # width eksplisit di tiap kolom — DataTable Textual gak otomatis nge-resize ulang
            # kolom pas isi cell di-update lewat update_cell() (cuma auto-fit pas add_row()
            # pertama kali), jadi kalau gak di-set manual, value yang lebih panjang dari
            # placeholder awal ("-") bakal kepotong pas nilainya masuk belakangan.
            columns = [
                ("Account", "account", 9),
                ("Email", "email", 24),
                ("Session", "session", 24),
                ...
                ("Settle", "settle", 14),
                ("Updated", "updated", 9),
                ("Last Message", "last", 60),
            ]
            for name, key, width in columns:
                table.add_column(name, key=key, width=width)
            ...
            for acc_id in self.account_ids:
                ...
                table.add_row(
                    ...
                    st["round"], st["votes"], st["window"], st["settle"], st["updated"], st["last"],
                    key=acc_id,
                )
        ...
        def refresh_from_files(self):
            for acc_id in self.account_ids:
                ...
                if state:
                    raw_status = state.get("last_status") or st.get("status") or "-"
                    st["status"] = _status_label(raw_status)
                    st["round"] = _shorten_round(state.get("last_round_id") or st.get("round") or "-")
                    st["votes"] = _votes_label(state)
        ...
        def update_account_row(self, acc_id):
            ...
            if not st:
                return
            for key in ["account", "email", "session", "worker", "status", "round", "votes", "window", "settle", "updated", "last"]:
                try:
        ...
        def parse_and_apply_line(self, line):
            ...
            if acc_id not in self.stats and acc_id != "manager":
                self.stats[acc_id] = {
                    ...
                    "status": "-", "round": "-", "votes": "-", "window": "-", "settle": "-",
                    "updated": "-", "last": "-",
                }
                self.logs_db[acc_id] = []
        ...
        def send_control(self, action: str, acc_id: str):
            if not self.process or self.process.poll() is not None or not self.process.stdin:
                self.add_log(acc_id, "manager belum jalan, tidak bisa kirim kontrol per-akun")
                return
            try:
                payload = json.dumps({"action": action, "account": acc_id}) + "\n"
                self.process.stdin.write(payload)
                self.process.stdin.flush()
            except Exception as e:
                self.add_log(acc_id, f"gagal kirim kontrol ke manager: {e}")
        ...
        def action_start_bot(self):
            acc_id = self.active_account
            if acc_id and acc_id != "ALL":
                if not (self.process and self.process.poll() is None):
                    self.add_log(acc_id, "manager belum jalan — pindah ke tab ALL (tekan A) lalu tekan S dulu")
                    return
                self.add_log(acc_id, f"▶️ start @{acc_id} diminta dari UI")
                self.send_control("start_account", acc_id)
                self.stats[acc_id]["worker"] = "Starting"
                self.update_account_row(acc_id)
            else:
                self.start_bot()
        ...
        def action_stop_bot(self):
            acc_id = self.active_account
            if acc_id and acc_id != "ALL":
                self.add_log(acc_id, f"⏹ stop @{acc_id} diminta dari UI")
                self.send_control("stop_account", acc_id)
                self.stats[acc_id]["worker"] = "Stopped"
                self.update_account_row(acc_id)
            else:
                self.add_log("ALL", "⏹ stop semua akun diminta")
                self.stop_bot_process()
                for aid in self.account_ids:
                    self.stats[aid]["worker"] = "Stopped"
                    self.update_account_row(aid)

        def action_restart_bot(self):
            acc_id = self.active_account
            if acc_id and acc_id != "ALL":
                if not (self.process and self.process.poll() is None):
                    self.add_log(acc_id, "manager belum jalan, tidak bisa restart akun ini")
                    return
                self.add_log(acc_id, f"🔄 restart @{acc_id} diminta dari UI (akun lain tidak ikut restart)")
                self.send_control("restart_account", acc_id)
                self.stats[acc_id]["worker"] = "Restarting"
                self.update_account_row(acc_id)
            else:
                self.action_restart_all_bot()

        def action_restart_all_bot(self):
            self.add_log("ALL", "🔄 restart SEMUA akun diminta")
            self.stop_bot_process()
            time.sleep(1)
            self.start_bot()
